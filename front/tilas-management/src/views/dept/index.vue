...
import { queryAllApi, addDeptApi, updateDeptApi, queryInfoApi, deleteDeptApi } from '@/api/dept'
...
const handleEdit = async (id) => {
        ...
        if (result.code === 1) {
            ...
            ElMessage.error(result.msg || '根据ID查询部门信息失败')
        }
}
...
const handleDelete = async (id) => {
        ...
        ElMessageBox.confirm('确定要删除该部门吗？', '提示', {
                ...
                if (result.code === 1) {
                        ...
                        ElMessage.error(result.msg || '部门删除失败')
                }
        })
}
const formTitle = ref('')
...
const formRules = {
        name: [
                ...
                { min: 2, max: 10, message: '部门名称长度必须在 2 - 10 之间', trigger: 'blur' }
        ]
}
//表单标签宽度
...
const save = async () => {
        await deptFormRef.value.validate(
                async valid => {
                        ...
                        if (result.code === 1) {
                                ElMessage.success(deptForm.value.id ? '部门修改成功' : '部门新增成功')
                                //关闭对话框
                                ...
                                ElMessage.error(result.msg || (deptForm.value.id ? '部门修改失败' : '部门新增失败'))
                        }
                }
        )
}
